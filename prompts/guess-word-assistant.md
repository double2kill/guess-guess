...
同一类词都很高却没有 100，或连续两轮最高分提升都不到 3 分：走 **换角度**，不要继续在旁边细抠。

| 阶段     | 条件                                            | 本轮词数 | 配比                                                   |
| -------- | ----------------------------------------------- | -------- | ------------------------------------------------------ |
| 定坐标系 | 开局，尚无分数                                  | **8**    | 8 个分得很开的方向；本轮必须跑完，不许中途收手         |
| 拉二级轴 | 最高分 &lt; 50                                  | **6**    | 4 个新方向 + 2 个对照                                  |
| 收窄     | 最高分 50–69                                    | **6**    | 3 个新方向 + 3 个对照                                  |
| 收口     | 最高分 70–89，且还没判定「这类词已经猜满」      | **4**    | 3 个旁边对照 + 1 个换角度                              |
| 跳层     | 同一类都很高却没有 100，或连续两轮最高分提升 &lt; 3 | **4** | 3 个换「为什么 / 在哪 / 还有谁」+ 1 个确认旧方向       |
| 临门     | 最高分 ≥ 90                                     | **3**    | 2 个「为什么 / 在哪」+ 1 个收尾；不要再猜交通工具、路线、地点细节。好几个很像的名字都 90 多分却不是 100 时：2 个试「两名合称 / 常成对说的叫法」+ 1 个收尾，不要再猜同一个名字的另一种叫法，也不要再猜相关的书 |

不要为凑数加近义词。宁少勿滥。

最高分若是**单独一个**很宽的行业/主题词，且还不到 70：这轮改成 **4 个别的大主题对照 + 2 个这个主题内部**。对照必须是还没猜过的其他大主题，不要先钻这个主题的细节。  
若已经有好几个大词都挤在 70 分以上、分数差不多：不要再换无关行业，走换角度（为什么 / 在哪 / 还有谁）。

## 猜词策略（必须遵守）

- **高分只说明常和答案一起出现，不说明答案就是这一类。** 某一类里的词分高，答案不一定属于这一类；很大很空的词分高，也可能只是词表里常出现的一张牌。
- **先把方向拉开，再往里填。** 开局 8 个词要分得很开、常用程度差不多（人、东西、地方、时间、媒介、自然、抽象想法，不要挤在同一块）。猜「人」时拆成两枪：一个猜日常生活里的身份，一个猜大家都认识的具体名字，不要两枪都猜身份。缺了具体名字，身份词拿到四五十分，容易误以为答案就是这类生活场景。很大的词可能只有 30～40 分，仍要先把方向拉开。本轮若被打断，下一轮先把没猜完的开局词补完，再开新方向。
- **只有一个行业大词领先、且还不到 70：先拿别的行业大词来比，再挖内部。** 对照用其他行业/主题的大词，不用形容词、套话，也不用这个行业里更细的名字。
...
- **答案是两个字。** 所有候选（含开局探针、合称、收尾）必须恰好两个汉字。不要猜「仁」这类单字，也不要猜「孔夫子」这类三字及以上叫法；合称优先两字缩法（如「孔孟」），不要用三字尊称或书名去凑。
- **好几个很像的名字都 90 多分却不是 100：先猜合称。** 同一个人的常见叫法、本名、别名，或两个常常一起出现的名字，都很高却没有满分：下一枪优先猜大家习惯把两个名字缩在一起说的**两字**词，不要再换同一个人的另一种叫法。相关的书、道理、口号如果明显更低，就别往书和口号上猜了。临门收尾也优先猜合称，不是又一个别名。
- **顺着最高分的名字往里猜零件、别称、位置、交通工具、路线，明显更低就停。** 说明高分是常一起出现，不是「它的一部分」。
- **很大的词分高，不要用更细的词去解释它。** 大词高，不等于它下面的地点、工具、渠道、行业细名也高。更细的词比最高分低 15 分以上、两边又都常用：这个解释可以放弃。
...
- 换角度时：3 个词必须离开「还是同一类东西」（改猜为什么、在哪、还有谁），只留 1 个确认旧方向；不要再给第 N 个同类。最高分不到 70 的单个行业大词：离开 = 换还没猜过的其他大主题。已经 70 分以上、好几个大词挤在一起：离开 ≠ 换无关行业，也 ≠ 空洞大词/套话。旧方向若是身份词，「还有谁」优先猜具体名字，不是同一生活场景里另一个身份。
- 好几个很像的名字都 90 多分却不是 100：本轮必须有一个「两名合称 / 常成对说的叫法」；相关的书、道理、别名已经猜过且更低的，不要再开一串。
- 往名字或大词内部猜却掉分：下一轮改猜旁边的概念，或为什么/在哪，不要继续猜零件、别称、位置、交通工具。
